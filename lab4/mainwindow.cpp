...
MainWindow::MainWindow(QWidget *parent)
    ...
    , commandProcessor_(new CommandProcessor(&manager_))  // Добавьте эту строку
...
{
    saveData();
    delete commandProcessor_;  // Добавьте эту строку
    delete ui;
}
...
void MainWindow::on_loadCommandsFileButton_clicked()
{
    currentCommandsFile_ = QFileDialog::getOpenFileName(this,
                                                        "Выберите файл с командами",
                                                        "",
                                                        "Текстовые файлы (*.txt);;Все файлы (*)");

    if (!currentCommandsFile_.isEmpty()) {
        statusBar()->showMessage("Выбран файл команд: " + currentCommandsFile_);
    }
}

void MainWindow::on_executeCommandsButton_clicked()
{
    if (currentCommandsFile_.isEmpty()) {
        QMessageBox::information(this, "Информация",
                                 "Сначала выберите файл с командами (кнопка 'Загрузить файл команд')");
        return;
    }

    QVector<CommandResult> results = commandProcessor_->executeCommandsFromFile(currentCommandsFile_);

    // Обновляем таблицу после выполнения команд
    refreshTable();
    saveData();

    showCommandResults(results);
}

void MainWindow::showCommandResults(const QVector<CommandResult>& results)
{
    QString log = commandProcessor_->getExecutionLog();
    ...
    QMessageBox msgBox;
    msgBox.setWindowTitle("Результат выполнения команд");
    msgBox.setText(log + summary);
    msgBox.setDetailedText(commandProcessor_->getExecutionLog());
    msgBox.exec();

    statusBar()->showMessage(QString("Команды выполнены. Успешно: %1, Ошибок: %2")
                                 .arg(successCount).arg(errorCount));
}
